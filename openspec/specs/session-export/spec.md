...
threshold AND which are not rejected. The threshold SHALL default to 1, so an
export with no threshold given carries every shot the user has rated at all
and no unrated ones, provided they are not rejected.
...
- **WHEN** a session has shots rated 0, 2 and 5 (none rejected), all with images on disk, and an export is requested with no threshold
- **THEN** the response is a ZIP containing exactly the two rated images, and the session folder is unchanged

#### Scenario: Rejected shots are excluded
- **WHEN** a session has a rejected shot rated 5, and an export is requested with a threshold of 1
- **THEN** the rejected shot is not included in the export
