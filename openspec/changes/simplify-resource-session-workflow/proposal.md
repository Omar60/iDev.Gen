...
- Expose browser import through a closed server-owned selection HTTP contract with safe projections, revision checks, replayable commit results and deterministic staging cleanup.
...
- Resolve the effective workflow server-side from the character default or an explicit Advanced override, freeze its verified identity, and make atomic guided creation idempotent across retries and concurrent duplicate requests.
...
- Make automatic preparation results, effective state, request/output evidence, provenance and dependency digests server-owned; retain validated manual completion only for manual or pre-authoring expert plans and prevent raw completion routes from bypassing that authority.
- Persist incremental progress through a closed recoverable-operation state machine, prevent concurrent duplicate authoring, support cancellation/recovery and verified snapshot-and-adaptation copy-forward into current revisions, and define downstream invalidation after edits. Detect mutable resource drift at review/submission and provide explicit dependency refresh without creative edits.
- Treat plan look/wardrobe as authoritative for resource sessions, project them into listing/search without mirroring writes into legacy session columns, and extend the existing feature flag and downgrade procedure to protect authoring metadata.
...
Backend: resource import/service/translation/preparation, persisted selection/operation/idempotency records, session-plan persistence, guided HTTP boundaries, resource-session projections and existing assistant transport. Frontend: Resources, resource draft helpers, a personal Looks surface and SessionView. Reuse the existing garment/outfit catalogue and its authored removal order; do not add clothing as a measured-cell dimension. Reuse existing dependencies and preserve legacy creation, catalogue gates, exact revisions, rollback behavior and runner/reference semantics.
