...
- [ ] 5.4 Judge an already-judged session through the screen against a verdict already known, and report whether the screen reproduces it **CLOSED WITH THE CHANGE AND NOT DONE - it needs somebody at the screen.** The control machinery this task would exercise is built and tested (5.3 above: `control: bool` on `JudgeShotIn`, the `if j.control:` branch that reads without writing, `buildJudgeDeck` and `computeAgreement`, and three backend tests including the negative verification). What is missing is the act of judging, and it cannot be scripted from here: the only way to make the screen produce an answer is to drive the browser against the live database, and a scripted click on the wrong panel silently edits a real session. The report the task asks for is also the operator's - whether the screen reproduces a verdict they already hold is a judgement about their own eyes. This is the same task as 7.5 of the catalogue-store-and-judge-repair change, written twice; doing it once answers both.
- [ ] 5.5 Time one batch of human judging against one batch of vision judging and record the real ratio, replacing the estimate in design.md **CLOSED WITH THE CHANGE AND NOT DONE - half of the measurement is a human with a stopwatch.** The vision half is runnable today (`scripts/judge_cell.py`), the human half is not, and a ratio with one arm estimated is the estimate already in design.md under a new name. The estimate stands and is marked as an estimate, which is the honest state; the task is worth re-opening the first time a real batch of human judging happens for another reason and can be timed for free.
...
- [x] 7.1 Remove the two inline camera examples from the instruction prose and verify the single-home test from 1.3 still passes with those two texts deleted from its `KNOWN_DUPLICATES` baseline, and no test asserting prompt text changes — the two examples sat in the `camera` field paragraph of `SHOOT_LINE_INSTRUCTION` (`frontend/src/kinds.js:1515`), and they were a second home for two catalogue wordings the app already deals onto the row of every photograph (`cameraPlan` at `frontend/src/enhance.js:418`, drawn from `positionsFor(manner)`). The sentence keeps its rule and loses its examples: it now says the position is handed to you on the row of each photograph like the framing below it, and to copy it word for word and write the framing after it. That is the same shape the framing paragraph two lines down already had, so the writer is not left without a model — it is told to copy the one it is given rather than to imitate one printed in the prose. `KNOWN_DUPLICATES` in `tests/test_one_home.py` loses `'Taken from her right side, her body in full profile'` and `'Taken from directly behind her'`, 11 entries down to 9, and the comment above the list records why rather than leaving the two names dangling. The assertion is an equality against that list, so it cannot pass vacuously: putting either text back in the prose without putting it back in the list fails. No test asserted this prose. The 20 test hits on those two strings (`tests/test_shoot_checks.py`, `tests/test_arrangements.py`, `tests/test_kiss_frames.py`, `tests/test_analyze_subject.py`) are all fixtures using the catalogue wordings as camera values, which is what they are for.
- [x] 7.2 Update `README.md` and the matching page under `docs/` with the judging screen and the composer path, including that strict mode refuses rather than repeats — the judging screen and the composer path were already written up (`README.md` § Composing from the catalogue and § Component Catalogue & Judging, `docs/judging.md` whole, `docs/sessions.md` § Composing from the catalogue); what neither page carried is the refusal. `README.md` gains a paragraph after the crop law: the two modes and what each draws, that neither draws a dead cell, that every check runs before any insertion so a request queues all of its photographs or none, that the 422 names the largest count that would have worked, and that strict queuing nothing on a young catalogue is the table being honest rather than the feature being broken. `docs/sessions.md` gains **A short pool is refused, never padded** under § Strict and exploratory: a trio is drawn at most once so a run of N fills N distinct cells, a pool that runs out before the count does is refused rather than repeated into, and the round-robin passes are named — the first pass repeats nothing, a later pass reuses a camera or an act only once the run has nowhere else to go. That last sentence is the one the task's phrasing could have got wrong: the run never repeats a TRIO, and it does reuse a value inside a slot, but only after a full pass.
- [x] 7.3 Run the full gates — `python -m pytest`, `npm --prefix frontend run build`, `npm --prefix frontend test` — and report the output rather than the summary — run after 7.1 and 7.2, on the merge commit plus those two.

  `python -m pytest --color=no` — `724 passed, 1 warning in 28.20s`. The one warning is the pre-existing pydantic `register` shadow at `backend/enhance.py:119`, unrelated to this change.

  `npm --prefix frontend run build` — `✓ built in 1.40s`; `dist/index.html 0.42 kB`, `dist/assets/index-BpUtxbo4.css 7.09 kB`, `dist/assets/index-BggT0fJ9.js 443.05 kB` (gzip 138.79 kB). The JS hash moved from `index-BEN7jLkB.js` and the byte count from 443.06 kB to 443.05 kB, which is 7.1 taking the two example strings out of the bundle; the CSS hash is unchanged.

  `npm --prefix frontend test` — vitest v4.1.11, `Test Files 9 passed (9)`, `Tests 126 passed (126)` in 751ms.
