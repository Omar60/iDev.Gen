...
- [ ] 3.1 Score a hit as "the reading picked is the family the line asked for", keeping the existing exact-key and family branches ahead of nothing, and verify with a test that an answer stored as a component key before this change still scores as it did. Write it as ONE reduction and not two: `_hit(slot, ans) = bool(ans) and _reduce(ans) == _reduce(drawn[slot])` is equivalent to the three branches on every case they cover, and a second function answering "do these name the same thing" is the shape every counting bug in this branch has had
- [ ] 3.1b Look up a family by `concept_key` only. `component.wording` is the sentence that goes in the prompt and no answer key can equal it, so `OR wording=?` is a branch that never fires — and it is the same wording-for-key confusion that shipped a component delete guard which matched nothing, ever. Verify with a test that a component whose wording happens to equal another component's family name does not make a wrong answer score as a hit
