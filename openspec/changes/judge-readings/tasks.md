...
- [ ] 1.2 Add `GET /api/readings?slot=&manner=&session_id=` returning the union (base plus that session's) and verify with a test that a base reading and a session reading both come back for that session and only the base one for another session
...
- [ ] 6.1 Write the base readings for `camera`, `act` and `framing` under `directed` for the current bench — including `front` for the camera slot, which the measured floor produces with an empty prompt — and verify the framing pass on session 308 opens with them
